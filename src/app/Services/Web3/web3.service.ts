...
export class Web3Service {
  ...
  public async web3login() {
    SAMPLEJSON = require('../../../../build/contracts/Sample.json')
    return new Promise(async (resolve, reject) => {
      ...
      if (window.ethereum || window.web3) {
        ...
        if (typeof SAMPLEJSON.networks[Net] === 'undefined') {
        ...
        this.AccountSubscription = this.RefreshedAccount.subscribe(async () => {
          ...
          if (
            ...
            const SampleInstance = new window.web3.eth.Contract(
              SAMPLEJSON.abi,
              SAMPLEJSON.networks[Network].address
            )
